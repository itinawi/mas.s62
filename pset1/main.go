...
func GenerateKey() (SecretKey, PublicKey, error) {
  ...
  for i := 0; i < 256; i++ {
    ...
    for j := 0; j < 32; j++ {
      bloc_1[j] = byte(rand.Int())
    }
    sec.OnePre[i] = bloc_1
    pub.OneHash[i] = bloc_1.Hash()

    for j := 0; j < 32; j++ {
      bloc_2[j] = byte(rand.Int())
    }
    // Made sure to change the One and Zero Pres
    sec.ZeroPre[i] = bloc_2
    pub.ZeroHash[i] = bloc_2.Hash()
  }
	// ===
	return sec, pub, nil
}
  ...
  for i := 0; i < 32; i++ {
    for j := 0; j < 8; j++ {
      ...
      if (int(msg[i]) & indices[j]) == 0 {
        sig.Preimage[curr_index] = sec.ZeroPre[curr_index]
      } else {
        sig.Preimage[curr_index] = sec.OnePre[curr_index]
      }
    }
  }
	// ===
...
func Verify(msg Message, pub PublicKey, sig Signature) bool {
  ...
  for i := 0; i < 32; i++ {
    for j := 0; j < 8; j++ {
      curr_index := (8*i) + j
      if (int(msg[i]) & indices[j])  == 0 {
        baseline_pub.Preimage[curr_index] = pub.ZeroHash[curr_index]
      } else {
        baseline_pub.Preimage[curr_index] = pub.OneHash[curr_index]
      }
    }
  }
